...
const dinero = (
  n //formateador para mostrar montos con moneda de dolar y ajustar el numero de decimales
) =>
...
export default function Home() {
  //Componente principal(Inicio del sistema)

  const [kpis, setKpis] = useState({
    //kpis del sistema para medir o anailizar el rendimiento de la empresa en base a los asientos ya agregados en la base de datos
    ingresos: 0,
    gastos: 0,
    utilidad: 0,
    saldoCaja: 0,
  ...
  const getRangoMes = () => {
    //obtiene el primer y ultimmo del mes para calcular el rango
    const hoy = new Date();
    ...
    const cargar = async () => {
      ...
      try {
        ...
        for (const key of Object.keys(mayor || {})) {
          //si mayor esta vacio se evita fallar el sistea con {}
          ...
          if (pref === "5") {
            //si es una cuenta de ingreso se suma lo que crecio en el haber
            ingresos += haber - debe;
          } else if (pref === "4") {
            // si es una cuenta de gasto/costos se crecio en el debe
            gastos += debe - haber;
          }

          if (id.startsWith("1101")) {
            //si la cuenta pertenece a Caja/Bancos se toma el saldo directo y se agrega al total de efectivo del panel
        ...
        const ultimos = (Array.isArray(asientos) ? asientos : [])
          ...
          .flatMap((a) =>
            (a.partidas || []).map((p) => ({
              //se desarman los asientos para mostrar sus movimientos
              fecha: a.fecha,
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-800">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-slate-200 bg-white shadow-sm">
        <div className="mx-auto max-w-7xl flex h-16 items-center justify-center px-4">
          <img
            src={logo}
            alt="Logo"
            className="h-10 w-10 mr-4 rounded-full shadow"
          />
      ...
      <main className="flex-1 pt-16 px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl w-full flex flex-col gap-8 h-full items-stretch justify-start py-6 md:py-8">
            ...
            <div className="grid md:grid-cols-2">
              <div className="flex flex-col justify-center p-8 md:p-12">
                ...
                <p className="mt-4 text-lg text-slate-600">
                  Maneja las finanzas de tu empresa de una manera fácil, rápida
                  y segura. Todo en un solo lugar.
                </p>
          ...
          <section className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {(() => {
              const utilidadNegativa = kpis.utilidad < 0;
              ...
              const cards = [
                ...
                {
                  label: utilidadNegativa ? "Pérdidas netas" : "Utilidad neta",
                  value: kpis.utilidad,
                  isUtilidad: true,
                  negative: utilidadNegativa,
                },
                {
                  label: "Saldo en caja",
                  value: kpis.saldoCaja,
                  isUtilidad: false,
                },
              ];

              return cards.map((kpi, idx) => {
                const labelClass =
                  kpi.isUtilidad && kpi.negative
                    ? "text-rose-700"
                    : "text-slate-500";
                const valueClass =
                  kpi.isUtilidad && kpi.negative
                ...
                return (
                  ...
                  >
                    <p className={`text-sm ${labelClass}`}>{kpi.label}</p>
            ...
            <ul className="space-y-2 text-slate-700">
              <li>
                <span className="font-semibold">
                  BALCACERES SILVESTRE, LUIS FELIPE
                </span>{" "}
                — Ingeniería en Desarrollo de Software, Tercer año
              </li>
              <li>
                <span className="font-semibold">CHÁVEZ SOLITO, RENÉ DAVID</span>{" "}
                — Ingeniería en Desarrollo de Software, Tercer año
              </li>
              <li>
                <span className="font-semibold">
                  CRUZ ALGARIN, KATHERINNE JEANNETTE
                </span>{" "}
                — Ingeniería en Desarrollo de Software, Tercer año
              </li>
              <li>
                <span className="font-semibold">
                  GUEVARA MARTINEZ, DENNIS ADEMIR
                </span>{" "}
                — Ingeniería en Desarrollo de Software, Tercer año
              </li>
              <li>
                <span className="font-semibold">
                  VILLEDA ALABI, MARIO EDGARDO
                </span>{" "}
                — Ingeniería en Desarrollo de Software, Tercer año
              </li>
            </ul>
      ...
      <footer className=" bg-slate-100 text-center text-sm text-slate-500 p-4">
        &copy; {new Date().getFullYear()} Conta Pro. Todos los derechos
        reservados.
      </footer>
    </div>
  );
